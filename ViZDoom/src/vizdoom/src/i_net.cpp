...
void SendAbort (void)
{
	Printf ("Inside Send Abort....");
	BYTE dis[2] = { PRE_FAKE, PRE_DISCONNECT };
	int i, j;

	if (doomcom.numnodes > 1)
	{
		Printf ("Inside doomcom.numnodes > 1");
		if (consoleplayer == 0)
		{
			...
			for (i = 1; i < doomcom.numnodes; ++i)
			{
				for (j = 4; j > 0; --j)
				{
					Printf ("Inside i,j loop. Before Presend");
					PreSend (dis, 2, &sendaddress[i]);
					Printf ("Inside i,j loop. Affter Presend");
				}
			}
		}
		else
...
void HostGame (int i)
{
	...
	doomcom.consoleplayer = 0;
	Printf ("Network Started.....");
	Printf ("Network Started....inside here...");
	Printf ("Console player number: %d\n", doomcom.consoleplayer);
	Printf ("Console player number: 0");

	Printf ("Modified ViZDoom.....");
	doomcom.numnodes = 1;
	Printf ("Before Send Abort....");
	atterm (SendAbort);
	Printf ("After Send Abort.....");
